...
function load_team(frm, from_date, to_date) {
    ...
    frappe.call('rpm_team_worklogs', {from_date, to_date}).then(r => {
        ...
        for (const log of data.logs) {
            ...
            for (const row of log.lines) html += `<tr><td>${esc(row.activity_type)}</td><td>${esc(row.work_item)}</td><td>${esc(row.item_code)} ${esc(row.item_name_snapshot)}</td><td>${row.record_quantity ? esc(row.quantity) : ''}</td><td>${esc(row.result)}</td><td>${esc(row.hours)}</td><td>${esc(row.note)}</td></tr>`;
