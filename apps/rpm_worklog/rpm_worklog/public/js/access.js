frappe.ui.form.on('RPM Work Log Access', {
    refresh(frm) {
        ...
        w.html(`<a href="/desk" class="btn btn-default">返回首頁</a>
        ...
        const load = async () => {
            ...
            try {
                ...
                w.find('.records').html(`<table class="table table-bordered"><thead><tr><th>選取</th><th>User</th><th>工號｜姓名</th><th>員工角色</th><th>主管角色</th><th>檢查</th></tr></thead><tbody>${rows.map((r,i) => `<tr><td><input type="checkbox" aria-label="選取 ${esc(r.user)}" data-index="${i}" ${r.reasons.length ? 'disabled' : ''}></td><td>${esc(r.user)}<br>${esc(r.full_name)}</td><td>${esc(r.employee_label)}</td><td>${r.employee_role ? '有' : '未開通'}</td><td>${r.manager_role ? '有' : '未開通'}</td><td>${esc(r.reasons.join('；') || (r.ready ? '本人權限已配置' : '可開通／補齊本人權限'))}</td></tr>`).join('')}</tbody></table>`);
        ...
        const enroll = mode => {
            ...
            frappe.confirm(`將為 ${users.length} 人開通${mode === 'manager' ? '主管（可查看及審核直屬員工）' : '員工（填寫本人紀錄）'}權限，是否繼續？`, async () => {
                if (busy) return;
                busy = true; w.find('button').prop('disabled', true);
                try {
                    const {message:results} = await frappe.call({method:'rpm_worklog.access.enroll', type:'POST', args:{users,mode}});
