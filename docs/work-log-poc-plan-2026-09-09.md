...
> 後續研究整合：本文件保留實測與歷史提案。P1 直接加 Timesheet 欄位的順序已改為先做 [ADR-001 模型決策](adr-001-work-log-model.md)；新工作以 [需求清單](requirements-backlog.md) 與 [下一輪驗收](next-poc-acceptance.md) 為準。Completed 可保存不代表完整工作結果語意已驗證。
